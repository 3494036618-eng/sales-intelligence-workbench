...
项目只有一种运行方式：连接真实 Provider 与 AI Native 应用开发底座（Supabase），并在配置或依赖不完整时失败关闭。专业数据集（DataPro）、豆包搜索（联网搜索）、Agent 记忆（OpenViking）或模型失败时，依赖该能力的操作会明确失败，不会生成演示数据、固定报告或静态替代结果。生成前必须由专业来源中的法定名称或统一社会信用代码锚定目标主体；品牌、集团或简称来源只能支持明确标注主体边界的相关动态。报告不设置脱离当前证据的固定引用条数配额，也不会为了凑数量引入弱相关页面；系统按独立来源去重，并根据本次实际可用证据自适应确定整份报告的覆盖目标，最多覆盖 5 个独立来源，可靠来源较少时生成更短的报告。每个事实段落只引用能够直接支撑该段内容的来源。注册资本、营收、净利润、融资、估值及明确司法/处罚事实必须满足双来源规则，关键数字不一致时不能自行选择。来源区按专业数据集（DataPro）和豆包搜索（联网搜索）分组：专业数据集可展开逐字段核验，豆包搜索返回的联网结果显示网页标题、来源站点、发布时间和原文链接；来源没有可核验日期时明确显示“未标注发布时间”，不会用档案生成时间冒充资料时间。

档案 Agent 使用 Agent Plan Responses API 的强制函数调用。系统先按主体、经营、近期事件、风险、招采项目和来源独立性评估初始检索结果，只针对缺口执行有界补充检索；每个已完成查询都会写入内部检查点，可重试故障只继续未完成查询。随后一次提交固定六章节的完整内容规划：前五章规划可直接进入正文的事实或销售判断，“建议行动”单独规划具体动作，每章固定一个完整段落，每段同时绑定白名单来源和从来源摘要逐字复制的连续证据片段。六章正文和最终引用由服务端按已通过门禁的计划条目确定性组装，模型不能二次自由改写、换引、补引或删除章节。服务端独立检查结构、法定主体锚点、品牌/集团边界、语句完整性、日期/数值/命名实体支持、逐事实直接证据和高风险事实，不设置整份报告最低来源数量。专业数据集（DataPro）、豆包搜索（联网搜索）和模型传输层只对可重试的临时故障做有界重试；证据未变化时跳过模型，证据变化但最终正文及实际使用来源未变化时不保存重复版本。“资料截至”按最终实际引用及其中可核验的完整事件日期计算，不使用生成时间冒充资料时间。整个 Agent 最多调用模型 3 次（首次完整规划和最多两次定点修订，首次合格立即停止）；成功结果必须保留六个固定章节，每章至少有一个完整句子和可核验引用，仍不合格则任务失败且不保存新档案。运行时不使用文本 JSON 修补，不会在模型失败后拼接替代报告，也不会把旧的低质量档案用模板包装成新报告。完整协议和验收门槛见[档案 Agent 工程设计](docs/architecture/dossier-agent.md)。
...
- 当前持久化异步队列覆盖档案生成和 Agent 记忆（OpenViking）批量同步；前端飞书导入使用后端进程内受控任务，服务重启后任务进度不会恢复，但已成功写入的 OpenViking 正文和 AI Native 应用开发底座（Supabase）同步元数据不会丢失。首个稳定版还需把导入任务迁入持久化队列，并完成高可用 Worker 和容量压测。
